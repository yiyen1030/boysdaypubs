...
```
├── css/
│   └── main.css            # 自訂樣式（地圖濾鏡、Offcanvas、輪播、標記）
├── data/
│   ├── data.json           # 餐廳資料（13 間）
│   ├── pubs.json           # PUB 酒吧資料（8 間）
│   └── order.json          # 自訂顯示排序（id 陣列）
├── js/
│   └── script.js           # 全部互動邏輯
...
- 中心點：台南東門城（22.9871, 120.2174）
- **粉紅圓點** — 推薦餐廳；選入行程後轉為**琥珀色**
- **紫色圓點** — PUB 酒吧
- **藍色脈衝點** — 使用者目前位置（點擊 📍 定位按鈕觸發）

### 左側列表
- 從 `data.json` 與 `pubs.json` 動態載入，依 `order.json` 排序
- 點擊卡片 → 地圖移動至該店並開啟 Popup，卡片高亮（藍色左邊框）
- 點擊地圖標記 → 左側對應卡片自動捲動並高亮
- 餐廳與 PUB 以分隔線區隔，PUB 顯示紫色分類標籤

### 行程購物車
- 點擊 **+ 行程** 將餐廳或 PUB 加入今晚行程（上限 3 間）
- 選擇 2 間以上即可產生 Google Maps 步行路線
- 支援混合規劃（例如 2 間餐廳 + 1 間 PUB）

### 店家預覽（Offcanvas）
- 點擊 **👁 預覽** 從右側滑入面板
- 上方照片輪播（左右切換 + 圓點指示）
- 下方：評分／價位／距離、地址／電話、描述、招牌料理（餐廳）或招牌酒款（PUB）
...
| `tags` | Array | 搜尋標籤 |
| `desc` | String | 店家簡介 |
| `images` | Array | 照片（`src` + `type`），使用 Google Maps 圖片 URL（`=s1200-k-no`） |
| `beer` | Boolean | 是否提供啤酒 |
...
調整陣列內的 id 順序即可變更左側列表顯示順序，不影響 `data.json`。
